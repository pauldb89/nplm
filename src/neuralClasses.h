...
class Linear_layer {
 ...
 public:
  ...
  Linear_layer(int rows, int cols) {
    resize(rows, cols);
  }
  ...
  void fProp(
      const USCMatrix<ScalarIn> &input,
  ...
  void bProp(
      const MatrixBase<DerivedGOut> &input,
...
class Output_word_embeddings {
 ...
 public:
  ...
  Output_word_embeddings(int rows, int cols) {
    resize(rows, cols);
  }
  ...
  template <typename DerivedGOut, typename DerivedGIn>
  void bProp(
      const MatrixBase<DerivedGOut> &input_bProp_matrix,
  ...
  void computeGradient(
      ...
      double momentum) {
    //not sure if we want to use momentum here
    // W is vocab_size x output_embedding_dimension
    // b is vocab_size x 1
  ...
  void computeGradientAdagrad(
    ...
    uscgemv(
        learning_rate, gradient_output,
        Matrix<double,Dynamic,1>::Ones(weights.cols()), b_gradient);
...
class Input_word_embeddings {
 ...
 public:
  ...
  Input_word_embeddings(int rows, int cols, int context) {
